...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    ...
    cart.forEach((item) => {
      ...
      sum = sum + (priceNumber * item.quantity)
    })
    return sum
  };
  ...
  const handleIncrement = (item) => {
    dispatch(updateQuantity({ name: item, quantity: 1 }));
  };

  const handleDecrement = (name) => {
    const itemData = cart.find(item => item.name === name)
    if (itemData.quantity > 1) {
      dispatch(
        updateQuantity({ name: name, quantity: - 1 })
      );
    } else {
  ...
  const calculateTotalCost = (item) => {
    const priceNumber = Number(item.cost.replace(/[^0-9.-]+/g, ''));
    return item.quantity * priceNumber
  };
